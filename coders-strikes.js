...
class Pod extends Unit {
  ...
  playAngleThrust(a, t) {
	...
	checkpoint() {
		return new Checkpoint(this.nextCheckpointId, this.checkPointX, this.checkPointY, 600);
	}
	score() {
...
function play(pods, checkpoints) {
	// This tracks the time during the turn. The goal is to reach 1.0
	var t = 0.0;
	...
	while (t < 1.0) {
		...
		for (var i = 0; i < pods.length; ++i) {
			...
			// It is unnecessary to check all checkpoints here. We only test the pod's next checkpoint.
			// We could look for the collisions of the pod with all the checkpoints, but if such a collision happens it wouldn't impact the game in any way
			var col = pods[i].collision(checkpoints[pods[i].nextCheckpointId]);
		...
		if (prevCollision !== null && prevCollision.t === 0 && firstCollision.t === t &&
			prevCollision.a == firstCollision.a &&
			prevCollision.b === firstCollision.b) {
			firstCollision = null; //ignore it since we've already seen it with zero t and same two units
		}
...
function test(pods, checkpoints, x, y, thrust) {
	for (var i = 0; i < pods.length; ++i) {
		pods[i].rotate(new Point(x, y));
		pods[i].boost(thrust);
	}
...
var d = new Unit('1', 1, 1, 2, 12, 12);
var e = new Unit('2', 2, 2, 3, -1, -1);
*/
//onsole.log(d.collision(e).print());

//initial angle of the pod: 0 

/*var angle = pod.getAngle(b);
var diffAngle = pod.diffAngle(b)
console.log('angle:' + angle + ' diffAngle:' + diffAngle);

pod.calculateAngle(a, -180);
console.log('self angle:' + pod.angle); */

var checkpoint1 = new Checkpoint('11', 6500, 4500, 600);
var checkpoint2 = new Checkpoint('12', 13500, 2000, 600);
var pod = new Pod('1', 100, 100, 400, 0, 0, 45, checkpoint1.id, checkpoint1.x,
	checkpoint1.y, 0, 100, '3', false);
var angle = pod.getAngle(checkpoint1);
var diffAngle = pod.diffAngle(checkpoint1)
console.log('angle:' + angle + ' diffAngle:' + diffAngle);
var currentCheckIndex = 0;
var checkpointOrder = ['11', '12', '11', '12']

var pods = [];
pods.push(pod);
var checkpoints = {};
checkpoints[checkpoint1.id] = checkpoint1;
checkpoints[checkpoint2.id] = checkpoint2;

//console.log('begin position: ' + pods[0].print());
//test(pods, checkpoints, 5000, 4000, 100);
//console.log('new position: ' + pods[0].print());
/*
var move = new Move(25, 100);
for (var i=0; i<100; i++) {
  move.mutate(1);
...
//making 100 moves
for (var k = 0; k < 50; k++) {
  var checkpoint = checkpoints[checkpointOrder[currentCheckIndex]];
  console.log('pod position: ' + pods[0].print());
  console.log('angle:' + pods[0].getAngle(checkpoint) + ' diffAngle:' + pods[0].diffAngle(checkpoint));
	var solutions = generatePopulation(pods, checkpoints);
  var scores = scoreSolutions(solutions);
  console.log('original ai score:' + scores[4]);
	var sum = 0;
    ...
    solution.setEnv(clonePods(pods), checkpoints);
		...
		if (score > scores[minScoreIndex]) {
			solutions[minScoreIndex] = solution; //replace previous low score with this solution.
      //update scores
	...
	console.log('Total simulation runs: ' + sum);
	var bestScoreIndex = findMaxScore(scores);
  console.log('Best score: ' + scores[bestScoreIndex]);
  
  var bestSolution = solutions[bestScoreIndex];
	for (var i = 0; i < bestSolution.moves.length; i++) {
		console.log(bestSolution.moves[i].print());
	}
  pods[0].playAngleThrust(bestSolution.moves[0].angle, bestSolution.moves[0].thrust); //applying the move

  if (pods[0].checked > 0) {
    console.log('we hit the checkpoint ' + pods[0].nextCheckpointId + ' in ' + k + ' turns');
    if (currentCheckIndex === checkpointOrder.length-1) {
      //we are done
      console.log('that was the last checkpoing, victory in ' + k + ' turns');
      break;
    } else {
      pods[0].checked = 0; //reset checked
      pods[0].nextCheckpointId = checkpoingOrder[++currentCheckIndex];
      pods[0].checkPointX = checkpoints[currentCheckIndex].x;
      pods[0].checkPointY = checkpoints[currentCheckIndex].y;
      continue;
    }
  }
}
...
function generatePopulation(pods, checkpoints) {
	var solutions = [];
	for (var j = 0; j < 4; j++) {
		var moves = [];
  ...
  var diffAngle = pods[0].diffAngle(checkpoint);
  var moves = [];
  for (var i=0; i<6; i++) {
    var angle = diffAngle;
...
//now we need to calculate pod angle based on a point and diffangle.